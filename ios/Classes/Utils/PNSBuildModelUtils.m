...
__attribute__((constructor)) static void InitAliAuthColors() {
  kAliAuthDarkGrayColor = [UIColor colorWithRed:26.0 / 255.0
                                          green:26.0 / 255.0
                                           blue:26.0 / 255.0
                                          alpha:1.0];
}
...
+ (TXCustomModel *)buildModelWithStyle:(NSDictionary *)dict
  ...
  model = [self buildModelOption:dict
                          target:target
                           style:style
                        selector:selector];
  return model;
...
+ (TXCustomModel *)buildFullScreenPortraitModel:(NSDictionary *)viewConfig
  ...
  // JSON -> TXCustomModel
  // TXCustomModel *modelConfig = [TXCustomModel
  // mj_objectWithKeyValues:viewConfig];

  // 1状态栏START
  model.prefersStatusBarHidden =
      [viewConfig boolValueForKey:@"isStatusBarHidden" defaultValue:NO];
  bool isLightColor = [viewConfig boolValueForKey:@"lightColor"
                                     defaultValue:NO];
  model.preferredStatusBarStyle =
      isLightColor ? UIStatusBarStyleLightContent : UIStatusBarStyleDefault;
  // 1状态栏ENd

  /// 导航设置
  model.navIsHidden = [viewConfig boolValueForKey:@"navHidden" defaultValue:NO];
  model.navColor = [self getColor:[viewConfig stringValueForKey:@"navColor"
                                                   defaultValue:@"#3971fe"]];

  model.navTitle = [[NSAttributedString alloc]
      initWithString:[viewConfig stringValueForKey:@"navText"
                                      defaultValue:@"一键登录"]
          attributes:@{
            NSForegroundColorAttributeName : UIColor.whiteColor,
            NSFontAttributeName : [UIFont
                systemFontOfSize:[viewConfig floatValueForKey:@"navTextSize"
                                                 defaultValue:20.0]]
          }];

  /// 返回按钮 START
  bool isHiddenNavBack = [viewConfig boolValueForKey:@"navReturnHidden"
                                        defaultValue:NO];
  model.hideNavBackItem = isHiddenNavBack;
  /// 动态读取assets文件夹下的资源
  UIImage *navBackImage = [self
      changeUriPathToImage:[viewConfig stringValueForKey:@"navReturnImgPath"
                                            defaultValue:nil]];
  ...
  if (!isHiddenNavBack) {
    ...
    model.navBackButtonFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          ...
          imageView.frame =
              CGRectMake(CGRectGetMinX(frame), CGRectGetMaxY(frame),
                         CGRectGetWidth(frame), CGRectGetHeight(frame));
          frame.origin.y = [viewConfig floatValueForKey:@"navReturnOffsetY"
                                           defaultValue:5];
          frame.origin.x = [viewConfig floatValueForKey:@"navReturnOffsetX"
                                           defaultValue:15];

          frame.size.width = [viewConfig floatValueForKey:@"navReturnImgWidth"
                                             defaultValue:40];
          frame.size.height = [viewConfig floatValueForKey:@"navReturnImgHeight"
                                              defaultValue:40];
          return frame;
        };
  }
  ...
  /// 协议页面导航设置
  model.privacyNavColor =
      [self getColor:[viewConfig stringValueForKey:@"webNavColor"
                                      defaultValue:@"#000"]];
  UIImage *privacyNavBackImage = [self
      changeUriPathToImage:[viewConfig stringValueForKey:@"webNavReturnImgPath"
                                            defaultValue:nil]];
  if (privacyNavBackImage != nil) {
    model.privacyNavBackImage = privacyNavBackImage;
  }
  model.privacyNavTitleFont =
      [UIFont systemFontOfSize:[viewConfig floatValueForKey:@"webNavTextSize"
                                               defaultValue:18]];
  model.privacyNavTitleColor =
      [self getColor:[viewConfig stringValueForKey:@"webNavTextColor"
                                      defaultValue:@"#000"]];

  /// logo 设置
  model.logoIsHidden = [viewConfig boolValueForKey:@"logoHidden"
                                      defaultValue:NO];
  UIImage *image =
      [self changeUriPathToImage:[viewConfig stringValueForKey:@"logoImgPath"
                                                  defaultValue:nil]];
  if (image != nil) {
    /// logo 默认水平居中
    model.logoFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          frame.size.width = [viewConfig floatValueForKey:@"logoWidth"
                                             defaultValue:80];
          frame.size.height = [viewConfig floatValueForKey:@"logoHeight"
                                              defaultValue:80];
          frame.origin.y = [viewConfig
              floatValueForKey:@"logoOffsetY"
                  defaultValue:screenSize.height > screenSize.width ? 30 : 15];
          frame.origin.x =
              (superViewSize.width - [viewConfig floatValueForKey:@"logoWidth"
                                                     defaultValue:80]) *
              0.5;
          return frame;
        };
  ...
  /// slogan 设置 START
  //  model.sloganIsHidden = [viewConfig boolValueForKey: @"sloganHidden"
  //  defaultValue: NO]; model.sloganText = [
  //    [NSAttributedString alloc]
  //    initWithString: [viewConfig stringValueForKey: @"sloganText"
  //    defaultValue: @"一键登录欢迎语"]
  //        attributes: @{
  //         NSForegroundColorAttributeName: [self getColor: [viewConfig
  //         stringValueForKey: @"sloganTextColor" defaultValue: @"#555555"]],
  //         NSFontAttributeName: [
  //            UIFont systemFontOfSize: [viewConfig floatValueForKey:
  //            @"sloganTextSize" defaultValue: 19]
  //         ]
  //        }
  //  ];
  //  model.sloganFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
  //  CGRect frame) {
  //    if (screenSize.height > screenSize.width) {
  //      frame.origin.y = [viewConfig floatValueForKey: @"sloganOffsetY"
  //      defaultValue: 20 + 80 + 20]; return frame;
  //    } else {
  //      return CGRectZero;
  //    }
  //  };
  ...
  /// number 设置 START
  model.numberColor =
      [self getColor:[viewConfig stringValueForKey:@"numberColor"
                                      defaultValue:@"#555"]];
  model.numberFont =
      [UIFont systemFontOfSize:[viewConfig floatValueForKey:@"numberSize"
                                               defaultValue:17]];
  model.numberFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
          frame.origin.y = [viewConfig floatValueForKey:@"numFieldOffsetY"
  ...
  model.loginBtnText = [[NSAttributedString alloc]
      initWithString:[viewConfig stringValueForKey:@"logBtnText"
                                      defaultValue:@"一键登录欢迎语"]
          attributes:@{
            NSForegroundColorAttributeName :
                [self getColor:[viewConfig stringValueForKey:@"logBtnTextColor"
                                                defaultValue:@"#ff00ff"]],
            NSFontAttributeName : [UIFont
                systemFontOfSize:[viewConfig floatValueForKey:@"logBtnTextSize"
                                                 defaultValue:23]]
          }];
  /// 登录按钮背景设置
  NSArray *logBtnCustomBackgroundImagePath =
      [[viewConfig stringValueForKey:@"logBtnBackgroundPath"
                        defaultValue:@","] componentsSeparatedByString:@","];
  if (logBtnCustomBackgroundImagePath.count == 3) {
    // login_btn_normal
    UIImage *login_btn_normal =
        [self changeUriPathToImage:logBtnCustomBackgroundImagePath[0]];
    // login_btn_unable
    UIImage *login_btn_unable =
        [self changeUriPathToImage:logBtnCustomBackgroundImagePath[1]];
    // login_btn_press
    UIImage *login_btn_press =
        [self changeUriPathToImage:logBtnCustomBackgroundImagePath[2]];
    UIImage *defaultClick = [UIImage imageNamed:@"button_click"];
    UIImage *defaultUnClick = [UIImage imageNamed:@"button_unclick"];
    // fix '*** -[__NSPlaceholderArray initWithObjects:count:]: attempt to
    // insert nil object from objects[0]'
    if ((login_btn_normal != nil && login_btn_unable != nil &&
         login_btn_press != nil) ||
        (defaultClick != nil && defaultUnClick != nil)) {
      // 登录按钮设置
      model.loginBtnBgImgs = @[
        login_btn_normal ?: defaultClick, login_btn_unable ?: defaultUnClick,
  ...
  model.loginBtnFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
          frame.size.width = [viewConfig floatValueForKey:@"logBtnWidth"
                                             defaultValue:300];
          frame.size.height = [viewConfig floatValueForKey:@"logBtnHeight"
                                              defaultValue:40];
          frame.origin.y = [viewConfig floatValueForKey:@"logBtnOffsetY"
                                           defaultValue:170 + 30 + 20];
          frame.origin.x =
              (superViewSize.width - [viewConfig floatValueForKey:@"logBtnWidth"
                                                     defaultValue:300]) *
              0.5;
        } else {
  ...
  /// 协议设置 START
  NSString *protocolOneURL = [viewConfig stringValueForKey:@"protocolOneURL"
                                              defaultValue:nil];
  ...
  NSString *protocolTwoURL = [viewConfig stringValueForKey:@"protocolTwoURL"
                                              defaultValue:nil];
  ...
  NSString *protocolThreeURL = [viewConfig stringValueForKey:@"protocolThreeURL"
                                                defaultValue:nil];
  ...
  model.privacyAlertContentUnderline =
      [viewConfig boolValueForKey:@"privacyAlertProtocolNameUseUnderLine"
                     defaultValue:NO];
  // 协议1内容颜色
  model.privacyOneColor = kAliAuthDarkGrayColor;
  // 协议2内容颜色
  model.privacyTwoColor = kAliAuthDarkGrayColor;
  // 协议3内容颜色
  model.privacyThreeColor = kAliAuthDarkGrayColor;
  // 运营商协议内容颜色
  model.privacyOperatorColor = kAliAuthDarkGrayColor;

  // 二次协议1内容颜色
  model.privacyAlertOneColor = kAliAuthDarkGrayColor;
  // 二次协议2内容颜色
  model.privacyAlertTwoColor = kAliAuthDarkGrayColor;
  // 二次协议3内容颜色
  model.privacyAlertThreeColor = kAliAuthDarkGrayColor;
  // 二次运营商协议内容颜色
  model.privacyAlertOperatorColor = kAliAuthDarkGrayColor;
  ...
  model.privacyNavTitleFont =
      [UIFont fontWithName:@"PingFangSC-Regular"
                      size:[viewConfig floatValueForKey:@"webNavTextSize"
                                           defaultValue:12.0]];
  /** 返回按钮  */
  UIImage *webNavReturnImgPath = [self
      changeUriPathToImage:[viewConfig stringValueForKey:@"webNavReturnImgPath"
                                            defaultValue:nil]];
  ...
  model.privacyPreText =
      [viewConfig stringValueForKey:@"privacyBefore"
                       defaultValue:@"点击一键登录并登录表示您已阅读并同意"];
  model.privacySufText = [viewConfig stringValueForKey:@"privacyEnd"
                                          defaultValue:@"思预云用户协议，隐私"];
  model.privacyOperatorPreText =
      [viewConfig stringValueForKey:@"vendorPrivacyPrefix" defaultValue:@"《"];
  model.privacyOperatorSufText =
      [viewConfig stringValueForKey:@"vendorPrivacySuffix" defaultValue:@"》"];
  model.privacyAlertContentColors = @[ UIColor.blackColor, UIColor.blackColor ];
  model.privacyAlertButtonTextColors =
      @[ UIColor.whiteColor, UIColor.whiteColor ];

  /// 协议水平垂直设置
  model.privacyFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                    CGRect frame) {
    if ([viewConfig floatValueForKey:@"privacyOffsetY" defaultValue:-1] > -1) {
      frame.origin.y = [viewConfig floatValueForKey:@"privacyOffsetY"
                                       defaultValue:-1];
    }
    if ([viewConfig floatValueForKey:@"privacyOffsetY" defaultValue:-1] > -1) {
      frame.origin.x = [viewConfig floatValueForKey:@"privacyOffsetX"
                                       defaultValue:-1];
    }
    return frame;
  };
  // 0.2.3 - 1.12.4新增
  model.privacyVCIsCustomized =
      [viewConfig boolValueForKey:@"privacyVCIsCustomized" defaultValue:NO];
  // 是否使用授权页协议动画
  bool isPrivacyAnimation = [viewConfig boolValueForKey:@"isPrivacyAnimation"
                                           defaultValue:NO];
  ...
  /// 切换到其他标题 START
  BOOL changeBrnStatus = [viewConfig boolValueForKey:@"switchAccHidden"
                                        defaultValue:NO];
  model.changeBtnIsHidden = changeBrnStatus;
  if (!changeBrnStatus) {
    model.changeBtnTitle = [[NSAttributedString alloc]
        initWithString:[viewConfig stringValueForKey:@"switchAccText"
                                        defaultValue:@"切换到其他方式"]
            attributes:@{
              NSForegroundColorAttributeName : [self
                  getColor:[viewConfig stringValueForKey:@"switchAccTextColor"
                                            defaultValue:@"#555555"]],
              NSFontAttributeName : [UIFont
                  systemFontOfSize:[viewConfig
                                       floatValueForKey:@"switchAccTextSize"
                                           defaultValue:18]]
            }];
        ...
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          if (screenSize.height > screenSize.width) {
            return CGRectMake(10,
                              [viewConfig floatValueForKey:@"switchOffsetY"
  ...
  /// 页面弹出方向 START
  model.presentDirection = [viewConfig intValueForKey:@"presentDirection"
                                         defaultValue:0];
  /// 页面弹出方向 END

  /// 勾选统一按钮 START
  BOOL checkStatus = [viewConfig boolValueForKey:@"checkboxHidden"
                                    defaultValue:NO];
  model.checkBoxIsHidden = checkStatus;
  if (!checkStatus) {
    UIImage *unchecked = [self
        changeUriPathToImage:[viewConfig stringValueForKey:@"uncheckedImgPath"
                                              defaultValue:nil]];
    UIImage *checked = [self
        changeUriPathToImage:[viewConfig stringValueForKey:@"checkedImgPath"
                                              defaultValue:nil]];
    if (unchecked != nil && checked != nil) {
      model.checkBoxImages = @[ unchecked, checked ];
    }
  }
  model.checkBoxIsChecked = [viewConfig boolValueForKey:@"privacyState"
                                           defaultValue:NO];
  model.checkBoxWH = [viewConfig floatValueForKey:@"checkBoxHeight"
                                     defaultValue:17.0];
  /// 勾选统一按钮 END

  /// 自定义第三方按钮布局 START
  NSDictionary *customThirdView = [viewConfig dictValueForKey:@"customThirdView"
                                                 defaultValue:nil];
  if (customThirdView != nil) {
    NSMutableArray *customArrayView = [NSMutableArray array]; /// 空数组，有意义
    NSArray *customArray =
        [customThirdView arrayValueForKey:@"viewItemPath"
                             defaultValue:nil]; // 空数组，有意义
    NSArray *customNameArray =
        [customThirdView arrayValueForKey:@"viewItemName"
                             defaultValue:nil]; // 空数组，有意义
    if (customArray != nil && customArray.count > 0) {
      /// 第三方图标按钮的相关参数
      int width = [customThirdView intValueForKey:@"itemWidth" defaultValue:70];
      int height = [customThirdView intValueForKey:@"itemHeight"
                                      defaultValue:70];
      ...
      NSString *color = [viewConfig stringValueForKey:@"color"
                                         defaultValue:@"#3C4E5F"];

      for (int i = 0; i < customArray.count; i++) {
        CustomButton *button = [[CustomButton alloc]
        ...
        [button setTitleColor:[self getColor:color]
                     forState:UIControlStateNormal];
        [button setBackgroundImage:[self changeUriPathToImage:customArray[i]]
                          forState:UIControlStateNormal];
  ...
  /// 背景设置 START
  NSString *backgroundColor = [viewConfig stringValueForKey:@"backgroundColor"
                                               defaultValue:nil];
  if (![backgroundColor isEqual:nil]) {
    model.backgroundColor = [self getColor:backgroundColor];
  }
  NSString *backgroundImagePath =
      [viewConfig stringValueForKey:@"backgroundPath" defaultValue:nil];
...
+ (TXCustomModel *)buildFullScreenLandscapeModel:(NSDictionary *)dict
  ...
  //    [button1 addTarget:target1 action:selector1
  //    forControlEvents:UIControlEventTouchUpInside];
  //
  //    UIButton *button2 = [UIButton buttonWithType:UIButtonTypeSystem];
  //    [button2 setTitle:button2Title forState:UIControlStateNormal];
  //    [button2 addTarget:target2 action:selector2
  //    forControlEvents:UIControlEventTouchUpInside];
  ...
  //    };
  //    model.customViewLayoutBlock = ^(CGSize screenSize, CGRect
  //    contentViewFrame, CGRect navFrame, CGRect titleBarFrame, CGRect
  //    logoFrame, CGRect sloganFrame, CGRect numberFrame, CGRect loginFrame,
...
+ (TXCustomModel *)buildAlertPortraitMode:(NSDictionary *)viewConfig
  ...
  model.alertBarIsHidden = [viewConfig boolValueForKey:@"navHidden"
                                          defaultValue:NO];
  model.alertTitleBarColor =
      [self getColor:[viewConfig stringValueForKey:@"alertTitleBarColor"
                                      defaultValue:@"#3971fe"]];
  model.alertTitle = [[NSAttributedString alloc]
      initWithString:[viewConfig stringValueForKey:@"navText"
                                      defaultValue:@"一键登录"]
          attributes:@{
            NSForegroundColorAttributeName : UIColor.whiteColor,
  ...
  ;
  model.alertCloseItemIsHidden =
      [viewConfig boolValueForKey:@"alertCloseItemIsHidden" defaultValue:NO];

  UIImage *alertCloseImage = [self
      changeUriPathToImage:[viewConfig stringValueForKey:@"alertCloseImage"
                                            defaultValue:nil]];
  model.alertCloseImage =
      alertCloseImage ?: [UIImage imageNamed:@"icon_close_light"];
  ...
  model.alertCloseItemFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if ([self isHorizontal:screenSize]) {
          ...
          frame.origin.x = [viewConfig intValueForKey:@"alertCloseImageX"
                                         defaultValue:5];
          frame.origin.y = [viewConfig intValueForKey:@"alertCloseImageY"
                                         defaultValue:5];
          frame.size.width = [viewConfig intValueForKey:@"alertCloseImageW"
                                           defaultValue:30];
          frame.size.height = [viewConfig intValueForKey:@"alertCloseImageH"
                                            defaultValue:30];
          return frame;
        }
      };

  model.alertBlurViewColor =
      [self getColor:[viewConfig stringValueForKey:@"alertBlurViewColor"
                                      defaultValue:@"#000000"]];
  model.alertBlurViewAlpha = [viewConfig floatValueForKey:@"dialogAlpha"
                                             defaultValue:0.5];
  NSArray *alertCornerRadiusArray =
      [viewConfig arrayValueForKey:@"dialogCornerRadiusArray"
  ...
  model.privacyColors = @[
    [UIColor colorWithRed:153.0 / 255.0
                    green:153.0 / 255.0
                     blue:153.0 / 255.0
                    alpha:1.0],
    [UIColor colorWithRed:26.0 / 255.0
                    green:26.0 / 255.0
                     blue:26.0 / 255.0
                    alpha:1.0]
  ];
  ...
  // 切换到其他标题
  model.changeBtnIsHidden = [viewConfig boolValueForKey:@"switchAccHidden"
                                           defaultValue:NO];
  model.changeBtnTitle = [[NSAttributedString alloc]
      initWithString:[viewConfig stringValueForKey:@"switchAccText"
                                      defaultValue:@"切换到其他方式"]
  ...
  model.tapAuthPageMaskClosePage =
      [viewConfig boolValueForKey:@"tapAuthPageMaskClosePage" defaultValue:NO];
  /// 页面弹出方向 START
  model.presentDirection = [viewConfig intValueForKey:@"presentDirection"
                                         defaultValue:0];
  /// 页面弹出方向 END
  ...
  model.contentViewFrameBlock = ^CGRect(CGSize screenSize, CGSize contentSize,
    ...
    if ([self isHorizontal:screenSize]) {
      ...
      alertWidth = [viewConfig intValueForKey:@"dialogWidth"
                                 defaultValue:screenSize.width - alertX * 2];
      alertY = (screenSize.height - alertWidth * 0.5) * 0.5;
      alertHeight = [viewConfig intValueForKey:@"dialogHeight"
                                  defaultValue:screenSize.height - 2 * alertY];
    } else {
      alertWidth = [viewConfig intValueForKey:@"dialogWidth"
                                 defaultValue:screenSize.width / 2];
      alertHeight = [viewConfig intValueForKey:@"dialogHeight"
                                  defaultValue:screenSize.height / 2];
      alertX = [viewConfig intValueForKey:@"dialogOffsetX"
                             defaultValue:(TX_SCREEN_WIDTH - alertWidth) / 2];
      alertY = [viewConfig intValueForKey:@"dialogOffsetY"
                             defaultValue:(TX_SCREEN_HEIGHT - alertHeight) / 2];
    }
  ...
  if (customThirdView != nil) {
    ...
    if (customArray != nil && customArray.count > 0) {
      ...
      int space = [customThirdView intValueForKey:@"space" defaultValue:30];
      NSString *color = [customThirdView stringValueForKey:@"color"
                                              defaultValue:@"#3c4E5F"];
      for (int i = 0; i < customArray.count; i++) {
      ...
      model.customViewBlock = ^(UIView *_Nonnull superCustomView) {
        ...
        ////        [btn setImage:img
        /// forState:UIControlStateNormal];//button的填充图片
        //        [btn setTitleColor:[UIColor blackColor]
        //        forState:UIControlStateNormal]; btn.imageEdgeInsets =
        //        UIEdgeInsetsMake(- (btn.frame.size.height -
        //        btn.titleLabel.frame.size.height-
        //        btn.titleLabel.frame.origin.y),(btn.frame.size.width
        //        -btn.titleLabel.frame.size.width)/2.0f
        //        -btn.imageView.frame.size.width, 0, 0); btn.titleEdgeInsets =
        //        UIEdgeInsetsMake(btn.frame.size.height-btn.imageView.frame.size.height-btn.imageView.frame.origin.y,
        //        -btn.imageView.frame.size.width, 0, 0); [superCustomView
        //        addSubview: btn]; btn.frame = CGRectMake( 10, 50, 300, 200 );

        //        UITapGestureRecognizer* singleTap = [[UITapGestureRecognizer
        //        alloc] initWithTarget:self action:@selector(clickAllScreen:)];
  ...
  NSString *backgroundImagePath =
      [viewConfig stringValueForKey:@"pageBackgroundPath" defaultValue:nil];
  if (![backgroundImagePath isEqual:nil]) {
    model.backgroundImage = [self changeUriPathToImage:backgroundImagePath];
  }
  /// 背景设置 END
...
+ (TXCustomModel *)buildAlertLandscapeMode:(NSDictionary *)dict
  ...
  model.alertTitle = [[NSAttributedString alloc] initWithString:@"一键登录"
                                                     attributes:attributes];
...
+ (TXCustomModel *)buildSheetPortraitModel:(NSDictionary *)viewConfig
  ...
  //          }
  //    ];
  //    model.sloganFrameBlock = ^CGRect(CGSize screenSize, CGSize
  ...
  ///
  model.privacyOne =
      [[viewConfig stringValueForKey:@"appPrivacyOne"
                        defaultValue:nil] componentsSeparatedByString:@","];
  model.privacyTwo =
      [[viewConfig stringValueForKey:@"appPrivacyTwo"
                        defaultValue:nil] componentsSeparatedByString:@","];
  NSArray *privacyColors =
      [[viewConfig stringValueForKey:@"appPrivacyColor"
                        defaultValue:nil] componentsSeparatedByString:@","];
  if (privacyColors != nil && privacyColors.count > 1) {
    model.privacyColors =
        @[ [self getColor:privacyColors[0]], [self getColor:privacyColors[1]] ];
  }
  ...
  model.contentViewFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        ...
        frame.size.height = [viewConfig floatValueForKey:@"dialogHeight"
                                            defaultValue:460];
        frame.origin.x = 0;
...
+ (TXCustomModel *)buildVideoOrGifBackgroundModel:(NSDictionary *)viewConfig
                                         ...
                                         selector:(SEL)selector {
  return [self buildModelOption:viewConfig
                         target:target
                          style:style
                       selector:selector];
}
...
+ (TXCustomModel *)buildAlertFadeModel:(NSDictionary *)dict
  ...
  CABasicAnimation *entryAnimation =
...
+ (TXCustomModel *)buildAlertBounceModel:(NSDictionary *)dict
  ...
  CAKeyframeAnimation *entryAnimation =
  ...
  entryAnimation.timingFunctions = @[
    [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear],
    [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear],
  ...
  CAKeyframeAnimation *exitAnimation =
      [CAKeyframeAnimation animationWithKeyPath:@"transform.scale"];
  exitAnimation.values = @[ @1, @1.2, @0.01 ];
  exitAnimation.keyTimes = @[ @0, @0.4, @1 ];
  exitAnimation.timingFunctions = @[
...
+ (TXCustomModel *)buildFullScreenAutorotateModel:(NSDictionary *)dict
  ...
  model.supportedInterfaceOrientations =
    ...
    if (key && key.length > 0 && dict[key] != nil) {
      ...
      @try {
        ...
        if ([dict[key] isKindOfClass:[NSArray class]]) {
          NSArray *array = [dict arrayValueForKey:key
                                     defaultValue:[NSArray array]];
          NSMutableArray *mutableArray = [array mutableCopy];
          if ([key isEqual:@"privacyAlertCornerRadiusArray"] &&
              array.count == 4) {
          ...
          if ([key containsString:@"Path"]) {
            ...
            } else if ([key isEqual:@"privacyAlertBtnBackgroundImgPath"] &&
                       btn != nil) {
            ...
            if ([key isEqual:@"lightColor"]) {
              if (@available(iOS 13.0, *)) {
                // UIStatusBarStyleLightContent Light content, for use on dark
                // backgrounds UIStatusBarStyleDarkContent Dark content, for use
                // on light backgrounds
                [model setValue:@([dict boolValueForKey:key defaultValue:NO]
              ...
              [model
                  setValue:[self getColor:[dict stringValueForKey:key
                                                     defaultValue:@"#23effe"]]
                    forKey:newKey];
            }
          } else if ([key containsString:@"Size"]) {
            [model setValue:[UIFont systemFontOfSize:[dict floatValueForKey:key
                                                               defaultValue:17]]
                     forKey:newKey];
          } else {
...
#pragma mark 其他配置
  ...
  if (PNSBuildModelStyleGifBackground == style ||
      PNSBuildModelStyleVideoBackground == style) {
    backgroundView = [[PNSBackgroundView alloc] init];
    NSURL *backgroundUrl = [NSURL
        fileURLWithPath:
            [self changeUriToPath:[dict stringValueForKey:@"pageBackgroundPath"
                                             defaultValue:nil]]];
...
#pragma mark 2、导航栏
  NSAttributedString *attrs = [[NSAttributedString alloc]
      initWithString:[dict stringValueForKey:@"navText"
                                defaultValue:@"一键登录授权页面"]
          attributes:@{
            NSForegroundColorAttributeName :
                [self getColor:[dict stringValueForKey:@"navTextColor"
                                          defaultValue:@"#000000"]],
            NSFontAttributeName :
                [UIFont systemFontOfSize:[dict floatValueForKey:@"navTextSize"
                                                   defaultValue:20.0]]
          }];
  /// 弹窗标题设置不一致
  ...
  if (model.navIsHidden) {
    if (!model.hideNavBackItem) {
      /// 获取自定义的参数
      NSDictionary *customReturnBtn = [dict dictValueForKey:@"customReturnBtn"
                                               defaultValue:nil];
      /// 获取自定义的资源路径资源
      UIImage *navBackImagePath = [self
          changeUriPathToImage:[customReturnBtn stringValueForKey:@"imgPath"
                                                     defaultValue:nil]];
      if (customReturnBtn != nil && navBackImagePath != nil) {
        ...
        model.navBackButtonFrameBlock =
            ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
              ...
              imageView.frame =
                  CGRectMake(CGRectGetMinX(frame), CGRectGetMaxY(frame),
                             CGRectGetWidth(frame), CGRectGetHeight(frame));
              frame.origin.y = [customReturnBtn floatValueForKey:@"top"
                                                    defaultValue:5];
              frame.origin.x = [customReturnBtn floatValueForKey:@"left"
                                                    defaultValue:15];
              frame.size.width = [customReturnBtn floatValueForKey:@"width"
                                                      defaultValue:40];
              frame.size.height = [customReturnBtn floatValueForKey:@"height"
                                                       defaultValue:40];
              return frame;
            };
      } else {
        ...
        UIImage *navBackImage =
            model.navBackImage ?: [UIImage imageNamed:@"icon_close_light"];
        ...
        model.navBackButtonFrameBlock =
            ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
              ...
              imageView.frame =
                  CGRectMake(CGRectGetMinX(frame), CGRectGetMaxY(frame),
                             CGRectGetWidth(frame), CGRectGetHeight(frame));
              frame.origin.y = [dict floatValueForKey:@"navReturnOffsetY"
                                         defaultValue:5];
              frame.origin.x = [dict floatValueForKey:@"navReturnOffsetX"
                                         defaultValue:15];
              frame.size.width = [dict floatValueForKey:@"navReturnImgWidth"
                                           defaultValue:40];
              frame.size.height = [dict floatValueForKey:@"navReturnImgHeight"
                                            defaultValue:40];
              return frame;
            };
      }
...
#pragma mark 3、Logo
  if (!model.logoIsHidden && model.logoImage) {
    /// logo 默认水平居中
    model.logoFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                   CGRect frame) {
      ...
      frame.origin.y = [dict
          floatValueForKey:@"logoOffsetY"
              defaultValue:screenSize.height > screenSize.width ? 30 : 15];
      frame.origin.x =
          (superViewSize.width - [dict floatValueForKey:@"logoWidth"
                                           defaultValue:80]) *
          0.5;
      return frame;
    };
...
#pragma mark 4、Slogan
  if (!model.sloganIsHidden) {
    model.sloganText = [[NSAttributedString alloc]
        initWithString:[dict stringValueForKey:@"sloganText"
                                  defaultValue:@"思预云欢迎您使用一键登录"]
            attributes:@{
              NSForegroundColorAttributeName :
                  [self getColor:[dict stringValueForKey:@"sloganTextColor"
                                            defaultValue:@"#555555"]],
              NSFontAttributeName : [UIFont
                  systemFontOfSize:[dict floatValueForKey:@"sloganTextSize"
                                             defaultValue:19]]
            }];
    model.sloganFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          if (screenSize.height > screenSize.width) {
            frame.origin.y = [dict floatValueForKey:@"sloganOffsetY"
                                       defaultValue:20 + 80 + 20];
            return frame;
          } else {
            return CGRectZero;
          }
...
#pragma mark 6、登录按钮
  model.loginBtnText = [[NSAttributedString alloc]
      initWithString:[dict stringValueForKey:@"logBtnText"
                                defaultValue:@"一键登录"]
          attributes:@{
            NSForegroundColorAttributeName :
                [self getColor:[dict stringValueForKey:@"logBtnTextColor"
                                          defaultValue:@"#ff00ff"]],
            NSFontAttributeName : [UIFont
                systemFontOfSize:[dict floatValueForKey:@"logBtnTextSize"
                                           defaultValue:23]]
          }];
  NSArray *logBtnCustomBackgroundImagePath =
      [[dict stringValueForKey:@"logBtnBackgroundPath"
                  defaultValue:@","] componentsSeparatedByString:@","];
  if (logBtnCustomBackgroundImagePath.count == 3) {
    // login_btn_normal
    UIImage *login_btn_normal =
...
#pragma mark 7、切换到其他方式
  if (!model.changeBtnIsHidden) {
    model.changeBtnTitle = [[NSAttributedString alloc]
        initWithString:[dict stringValueForKey:@"switchAccText"
                                  defaultValue:@"切换到其他方式"]
            attributes:@{
              NSForegroundColorAttributeName :
                  [self getColor:[dict stringValueForKey:@"switchAccTextColor"
                                            defaultValue:@"#555555"]],
              NSFontAttributeName : [UIFont
                  systemFontOfSize:[dict floatValueForKey:@"switchAccTextSize"
                                             defaultValue:18]]
            }];
    model.changeBtnFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
...
#pragma mark 8、自定义控件区（如其他方式登录）
  NSDictionary *customThirdView = [dict dictValueForKey:@"customThirdView"
                                           defaultValue:nil];
  if (customThirdView != nil) {
    NSMutableArray *customArrayView = [NSMutableArray array]; /// 空数组，有意义
    NSArray *customArray =
...
#pragma mark 9、协议栏
  if (!model.checkBoxIsHidden) {
    UIImage *unchecked =
        [self changeUriPathToImage:[dict stringValueForKey:@"uncheckedImgPath"
                                              defaultValue:nil]];
    UIImage *checked =
        [self changeUriPathToImage:[dict stringValueForKey:@"checkedImgPath"
                                              defaultValue:nil]];
    if (unchecked != nil && checked != nil) {
      model.checkBoxImages = @[ unchecked, checked ];
    }
  ...
  model.privacyFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if ([dict floatValueForKey:@"privacyOffsetY" defaultValue:-1] > -1) {
          frame.origin.y = [dict floatValueForKey:@"privacyOffsetY"
                                     defaultValue:-1];
        }
        if ([dict floatValueForKey:@"privacyOffsetY" defaultValue:-1] > -1) {
          frame.origin.x = [dict floatValueForKey:@"privacyOffsetX"
                                     defaultValue:-1];
        }
        return frame;
      };
  model.privacyColors = @[
    [UIColor colorWithRed:153.0 / 255.0
                    green:153.0 / 255.0
                     blue:153.0 / 255.0
                    alpha:1.0],
    [UIColor colorWithRed:0x1A / 255.0
                    green:0x1A / 255.0
                     blue:0x1A / 255.0
                    alpha:1.0]
  ];
...
#pragma mark 10、弹窗样式
  if (PNSBuildModelStyleAlertPortrait == style ||
    ...
    model.alertCloseItemFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          if ([self isHorizontal:screenSize]) {
            ...
            frame.origin.x = [dict intValueForKey:@"alertCloseImageX"
                                     defaultValue:5];
            frame.origin.y = [dict intValueForKey:@"alertCloseImageY"
                                     defaultValue:5];
            frame.size.width = [dict intValueForKey:@"alertCloseImageW"
                                       defaultValue:30];
            frame.size.height = [dict intValueForKey:@"alertCloseImageH"
                                        defaultValue:30];
            return frame;
          }
        };
    ...
    if (PNSBuildModelStyleAlertPortrait == style) {
      CGFloat ratio = MAX(TX_SCREEN_WIDTH, TX_SCREEN_HEIGHT) / 667.0;
      // 实现该block，并且返回的frame的x或y大于0，则认为是弹窗谈起授权页
      model.contentViewFrameBlock = ^CGRect(CGSize screenSize,
                                            CGSize contentSize, CGRect frame) {
        CGFloat alertX = 0;
        CGFloat alertY = 0;
        CGFloat alertWidth = 0;
        CGFloat alertHeight = 0;
        if ([self isHorizontal:screenSize]) {
          alertX = ratio * TX_Alert_Horizontal_Default_Left_Padding;
          alertWidth = [dict intValueForKey:@"dialogWidth"
                               defaultValue:screenSize.width - alertX * 2];
          alertY = (screenSize.height - alertWidth * 0.5) * 0.5;
          alertHeight = [dict intValueForKey:@"dialogHeight"
                                defaultValue:screenSize.height - 2 * alertY];
        } else {
          alertWidth = [dict intValueForKey:@"dialogWidth"
                               defaultValue:screenSize.width / 2];
          alertHeight = [dict intValueForKey:@"dialogHeight"
                                defaultValue:screenSize.height / 2];
          alertX = [dict intValueForKey:@"dialogOffsetX"
                           defaultValue:(TX_SCREEN_WIDTH - alertWidth) / 2];
          alertY = [dict intValueForKey:@"dialogOffsetY"
                           defaultValue:(TX_SCREEN_HEIGHT - alertHeight) / 2];
        }
        return CGRectMake(alertX, alertY, alertWidth, alertHeight);
      };
      ...
      model.contentViewFrameBlock =
          ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
            frame.size.width = superViewSize.width;
            frame.size.height = [dict floatValueForKey:@"dialogHeight"
                                          defaultValue:460];
            frame.origin.x = 0;
            frame.origin.y = superViewSize.height - frame.size.height;
            return frame;
...
#pragma mark 10、二次弹窗
  model.privacyAlertTitleFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        ...
        CGFloat alertX = (superViewSize.width - alertWidth) * 0.5;
        // title宽度比弹窗小20pt
        CGFloat titleWidth = alertWidth - 20;
        // title起始x为弹窗x + 10，实现左右居中
        CGFloat titleX = alertX + 10;
        CGFloat titleY = [dict floatValueForKey:@"privacyAlertTitleOffsetY"
                                   defaultValue:20];
        return CGRectMake(titleX, titleY, titleWidth, frame.size.height);
      };
  /// 弹窗大小
  model.privacyAlertFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        CGFloat alertWidth = frame.size.width;
        CGFloat alertHeight = frame.size.height;
        CGFloat alertX = (superViewSize.width - alertWidth) * 0.5;
        CGFloat alertY = (superViewSize.height - alertHeight) * 0.5;
        return CGRectMake(alertX, alertY, alertWidth, alertHeight);
      };
  /// 确认按钮
  model.privacyAlertButtonFrameBlock =
      ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        // return CGRectMake(frame.origin.x,superViewSize.height - 50 - 20,
        // frame.size.width, 50);
        return CGRectMake(frame.origin.x, frame.origin.y + 15,
                          [dict floatValueForKey:@"privacyAlertBtnWidth"
                                    defaultValue:frame.size.width],
                          [dict floatValueForKey:@"privacyAlertBtnHeigth"
                                    defaultValue:50]);
      };
  model.privacyAlertButtonTextColors =
      @[ UIColor.whiteColor, UIColor.whiteColor ];
...
#pragma mark 屏幕方向
  if (model.privacyAlertIsNeedShow) {
    ...
    model.privacyAlertPrivacyContentFrameBlock =
        ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
          return CGRectMake(0, frame.origin.y + 10, frame.size.width,
                            frame.size.height);
        };
  }
...
/**
 16进制颜色转换为UIColor
 @param hexColor
 16进制字符串（可以以0x开头，可以以#开头，也可以就是6位的16进制）
 @return 16进制字符串对应的颜色
 */
...
+ (NSDictionary *)formatParmas:(NSDictionary *)parmas {
  ...
  for (int i = 0; i < keysArray.count; i++) {
    ...
    else if (![key containsString:@"logBtnBackgroundPath"] &&
             ([key containsString:@"path"] || [key containsString:@"Path"]) &&
             ![value containsString:@"http"] && ![value isEqual:nil] &&
             ![value isEqual:@""]) {
