...
+ (TXCustomModel *)buildFullScreenPortraitModel:(NSDictionary *)viewConfig
    ...
    model.preferredStatusBarStyle = isLightColor ? UIStatusBarStyleLightContent
    ...
    if (!isHiddenNavBack) {
        /// 自定义返回按钮
        model.navBackButtonFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                                CGRect frame) {
    ...
    if (image != nil) {
        ...
        model.logoFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
            ...
            frame.origin.y = [viewConfig floatValueForKey:@"logoOffsetY" defaultValue:
                    screenSize.height > screenSize.width ? 30 : 15];
            frame.origin.x = (superViewSize.width -
                              [viewConfig floatValueForKey:@"logoWidth" defaultValue:80]) * 0.5;
            return frame;
        };
...
//  };
    ...
    model.numberFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
            frame.origin.y = [viewConfig floatValueForKey:@"numFieldOffsetY" defaultValue:130 + 20 +
                                                                                          15];
        } else {
    ...
    if (logBtnCustomBackgroundImagePath.count == 3) {
        ...
        if ((login_btn_normal != nil && login_btn_unable != nil && login_btn_press != nil) ||
    ...
    model.loginBtnFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
            ...
            frame.origin.y = [viewConfig floatValueForKey:@"logBtnOffsetY" defaultValue:170 + 30 +
                                                                                        20];
            frame.origin.x = (superViewSize.width -
                              [viewConfig floatValueForKey:@"logBtnWidth" defaultValue:300]) * 0.5;
        } else {
    ...
    NSString *
    protocolThreeURL = [viewConfig stringValueForKey:@"protocolThreeURL" defaultValue:nil];
    ...
    // 协议1内容颜色
    model.privacyOneColor = [self getColor:@"#1A1A1A"];
    // 协议2内容颜色
    model.privacyTwoColor = [self getColor:@"#1A1A1A"];
    // 协议3内容颜色
    model.privacyThreeColor = [self getColor:@"#1A1A1A"];
    // 运营商协议内容颜色
    model.privacyOperatorColor = [self getColor:@"#1A1A1A"];

    // 二次协议1内容颜色
    model.privacyAlertOneColor = [self getColor:@"#1A1A1A"];
    // 二次协议2内容颜色
    model.privacyAlertTwoColor = [self getColor:@"#1A1A1A"];
    // 二次协议3内容颜色
    model.privacyAlertThreeColor = [self getColor:@"#1A1A1A"];
    // 二次运营商协议内容颜色
    model.privacyAlertOperatorColor = [self getColor:@"#1A1A1A"];
    ...
    if (customThirdView != nil) {
        ...
        if (customArray != nil && customArray.count > 0) {
            ...
            for (int i = 0; i < customArray.count; i++) {
                CustomButton *button = [[CustomButton alloc] initWithFrame:CGRectMake(0, 0, width,
                                                                                      height)];
            ...
            model.customViewLayoutBlock = ^(
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    NSInteger X = (contentWidth - (width * count + space * (count - 1))) / 2 +
                                  (space + width) * i; /// 两端评分
    ...
    NSString *
    backgroundImagePath = [viewConfig stringValueForKey:@"backgroundPath" defaultValue:nil];
...
+ (TXCustomModel *)buildAlertPortraitMode:(NSDictionary *)viewConfig
    ...
    model.alertCloseItemFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                             CGRect frame) {
    ...
    NSArray *alertCornerRadiusArray = [viewConfig arrayValueForKey:@"dialogCornerRadiusArray" defaultValue:@[
            @10, @10, @10, @10]];
...
//  };
    ...
    model.changeBtnFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
            return CGRectMake(10, [viewConfig floatValueForKey:@"switchOffsetY" defaultValue:18],
    ...
    model.contentViewFrameBlock = ^CGRect(CGSize screenSize, CGSize contentSize, CGRect frame) {
        ...
        if ([self isHorizontal:screenSize]) {
            ...
            alertWidth = [viewConfig intValueForKey:@"dialogWidth" defaultValue:screenSize.width -
                                                                                alertX * 2];
            alertY = (screenSize.height - alertWidth * 0.5) * 0.5;
            alertHeight = [viewConfig intValueForKey:@"dialogHeight" defaultValue:
                    screenSize.height - 2 * alertY];
        } else {
            alertWidth = [viewConfig intValueForKey:@"dialogWidth" defaultValue:screenSize.width /
                                                                                2];
            alertHeight = [viewConfig intValueForKey:@"dialogHeight" defaultValue:
                    screenSize.height / 2];
            alertX = [viewConfig intValueForKey:@"dialogOffsetX" defaultValue:
                    (TX_SCREEN_WIDTH - alertWidth) / 2];
            alertY = [viewConfig intValueForKey:@"dialogOffsetY" defaultValue:
                    (TX_SCREEN_HEIGHT - alertHeight) / 2];
        }
...
//        [superCustomView addGestureRecognizer:singleTap];
    ...
    NSString *
    backgroundImagePath = [viewConfig stringValueForKey:@"pageBackgroundPath" defaultValue:nil];
    if (![backgroundImagePath isEqual:nil]) {
        model.backgroundImage = [self changeUriPathToImage:backgroundImagePath];
    }
    /// 背景设置 END
                        ...
                        if ([key isEqual:@"lightColor"]) {
                            ...
                            13.0, *)) {
                                ...
                                [model setValue:@([dict boolValueForKey:key defaultValue:NO]
                                                  ...
                                                  : UIStatusBarStyleLightContent) forKey:newKey];
                            } else {
                                [model setValue:@([dict boolValueForKey:key defaultValue:NO]
                                                  ? UIStatusBarStyleDefault
                                                  : UIStatusBarStyleLightContent) forKey:newKey];
...
#pragma mark 其他配置
    ...
    if (PNSDIYAlertPortraitFade == style) {
        ...
        model.contentViewFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                              CGRect frame) {
            return contentViewFrame;
        };
    ...
    if (model.navIsHidden) {
        if (!model.hideNavBackItem) {
            ...
            if (customReturnBtn != nil && navBackImagePath != nil) {
                ...
                /// 动态读取assets文件夹下的资源
                UIImage *navBackImage = model.navBackImage
                                        ?: [UIImage imageNamed:@"icon_close_light"];
...
#pragma mark 3、Logo
    if (!model.logoIsHidden && model.logoImage) {
        ...
        model.logoFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
            ...
            frame.origin.y = [dict floatValueForKey:@"logoOffsetY" defaultValue:screenSize.height >
                                                                                screenSize.width
                                                                                ? 30 : 15];
            frame.origin.x =
                    (superViewSize.width - [dict floatValueForKey:@"logoWidth" defaultValue:80]) *
                    0.5;
            return frame;
        };
    }
...
#pragma mark 6、登录按钮
    ...
    model.loginBtnFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
        if (screenSize.height > screenSize.width) {
            ...
            frame.origin.x = (superViewSize.width -
                              [dict floatValueForKey:@"logBtnWidth" defaultValue:300]) * 0.5;
        } else {
            frame.origin.y = 110 + 30 + 20;
        }
        return frame;
...
#pragma mark 9、协议栏
    ...
    model.privacyColors = @[
            ...
            [UIColor colorWithRed:0x1A / 255.0 green:0x1A / 255.0 blue:0x1A / 255.0 alpha:1.0]];
#pragma mark 10、弹窗样式
    if (PNSBuildModelStyleAlertPortrait == style || PNSBuildModelStyleAlertLandscape == style ||
        PNSBuildModelStyleSheetPortrait == style) {
        ...
        if (PNSBuildModelStyleAlertPortrait == style) {
            ...
            model.contentViewFrameBlock = ^CGRect(CGSize screenSize, CGSize contentSize,
                ...
                if ([self isHorizontal:screenSize]) {
                    alertX = ratio * TX_Alert_Horizontal_Default_Left_Padding;
                    alertWidth = [dict intValueForKey:@"dialogWidth" defaultValue:screenSize.width -
                                                                                  alertX * 2];
                    alertY = (screenSize.height - alertWidth * 0.5) * 0.5;
                    alertHeight = [dict intValueForKey:@"dialogHeight" defaultValue:
                            screenSize.height - 2 * alertY];
                } else {
                    alertWidth = [dict intValueForKey:@"dialogWidth" defaultValue:screenSize.width /
                                                                                  2];
                    alertHeight = [dict intValueForKey:@"dialogHeight" defaultValue:
                            screenSize.height / 2];
                    alertX = [dict intValueForKey:@"dialogOffsetX" defaultValue:
                            (TX_SCREEN_WIDTH - alertWidth) / 2];
                    alertY = [dict intValueForKey:@"dialogOffsetY" defaultValue:
                            (TX_SCREEN_HEIGHT - alertHeight) / 2];
                }
                return CGRectMake(alertX, alertY, alertWidth, alertHeight);
            };
...
#pragma mark 10、二次弹窗
    model.privacyAlertTitleFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                                CGRect frame) {
    ...
    /// 确认按钮
    model.privacyAlertButtonFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize,
                                                 CGRect frame) {
...
#pragma mark 屏幕方向
    if (model.privacyAlertIsNeedShow) {
        ...
        model.privacyAlertPrivacyContentFrameBlock = ^CGRect(CGSize screenSize,
                                                             CGSize superViewSize, CGRect frame) {
...
+ (UIColor *)getColor:(NSString *)hexColor {
    ...
    return [UIColor colorWithRed:(float) (red / 255.0f) green:(float) (green /
...
+ (UIColor *)colorWithHexString:(NSString *)hexColor alpha:(float)opacity {
    NSString *
    cString = [[hexColor stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] uppercaseString];
...
+ (UIColor *)colorWithHex:(NSInteger)hex alpha:(CGFloat)alpha {
    return [UIColor colorWithRed:((float) ((hex & 0xFF0000) >> 16)) /
                                 255.0 green:
            ((float) ((hex & 0xFF00) >> 8)) / 255.0 blue:((float) (hex & 0xFF)) /
                                                         255.0 alpha:alpha];
}
...
+ (FlutterViewController *)flutterVC {
    ...
    if ([viewController isKindOfClass:[FlutterViewController class]]) {
        ...
        [self findCurrentViewController];
    }
}
